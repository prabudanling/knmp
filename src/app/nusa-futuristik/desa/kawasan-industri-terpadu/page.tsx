...
import { ArrowRight, Building2, ChevronRight, Cpu, Flower2, Gem, Globe, GraduationCap, Hammer, Heart, Home, Laptop, Package, Scale, Store, Target, Truck, Users, Wheat, Zap } from 'lucide-react'
...
const zones = [
  ...
  { title: 'Sentra UMKM Desa', description: 'Pusat pemberdayaan UMKM desa dengan pelatihan digital, branding, dan akses pasar nasional.', icon: Store, features: ['Training', 'Branding', 'Market Access'] },
  { title: 'Produk Lokal Unggulan', description: 'Pengembangan dan sertifikasi produk lokal unggulan desa untuk pasar premium dan ekspor.', icon: Gem, features: ['Certification', 'Premium Market', 'Export Ready'] },
  { title: 'Workshop & Keterampilan', description: 'Pusat pelatihan keterampilan dan kerajinan untuk meningkatkan kapasitas SDM desa.', icon: Hammer, features: ['Skills Training', 'Craft Center', 'Certification'] },
  { title: 'Micro Logistic Desa', description: 'Sistem logistik kecil desa untuk menghubungkan produksi lokal dengan pasar regional.', icon: Package, features: ['Local Hub', 'Regional Link', 'Smart Storage'] },
  { title: 'Cooperative Network', description: 'Jaringan koperasi desa yang saling terhubung untuk penguatan ekonomi bersama.', icon: Users, features: ['Coop Network', 'Shared Resource', 'Joint Marketing'] },
]

export default function DesaKawasanIndustriPage() {
  return (
      ...
      <section className="relative bg-gradient-to-r from-[#8B0000] to-[#6B0000] text-white py-12 md:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            <motion.div variants={fadeInUp}><Badge className="bg-white/20 text-white border-white/30 mb-4"><Building2 className="w-3.5 h-3.5 mr-1.5" />DESA FUTURISTIK</Badge></motion.div>
            <motion.h1 variants={fadeInUp} className="text-3xl md:text-4xl lg:text-5xl font-extrabold mb-4">Kawasan Industri Terpadu Desa</motion.h1>
            <motion.p variants={fadeInUp} className="text-lg text-white/80 max-w-2xl mx-auto">Sentra ekonomi desa berbasis BUMDes, UMKM, dan produk lokal unggulan yang terintegrasi dengan pasar nasional dan global.</motion.p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12 md:py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="mb-8">
            <motion.h2 variants={fadeInUp} className="text-2xl md:text-3xl font-extrabold text-[#1A1A1A] mb-2">Zona Industri Desa</motion.h2>
            <motion.p variants={fadeInUp} className="text-[#6B7280]">Berbagai zona industri desa yang memberdayakan ekonomi kerakyatan.</motion.p>
          </motion.div>
      ...
      <section className="py-12 bg-gradient-to-r from-[#008F3D] to-[#006F30] text-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
            <motion.h2 variants={fadeInUp} className="text-2xl md:text-3xl font-extrabold mb-4">Berkembang Bersama Ekonomi Desa</motion.h2>
            <motion.p variants={fadeInUp} className="text-white/80 max-w-xl mx-auto mb-6">Bergabung dengan KMN BERDIKARI dan akses peluang pemberdayaan ekonomi desa seluruh Indonesia.</motion.p>
