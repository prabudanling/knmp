'use client'

import { motion } from 'framer-motion'
import Link from 'next/link'
import { ArrowRight, Bike, Building2, Bus, Car, ChevronRight, Flower2, Globe, GraduationCap, Heart, Home, Laptop, Navigation, Scale, Target, Train, Truck, Warehouse, Wheat, Wifi, Zap } from 'lucide-react'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { Card, CardContent } from '@/components/ui/card'

const fadeInUp = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5 } } }
const staggerContainer = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }

const siblingPages = [
  { label: 'Kampung Pemerintahan Digital', href: '/nusa-futuristik/kota/proyek-strategis', icon: Laptop, color: '#3b82f6' },
  { label: 'Kampung Modal', href: '/nusa-futuristik/kota/kampung-modal', icon: Scale, color: '#f59e0b' },
  { label: 'Kampung Industri', href: '/nusa-futuristik/kota/kawasan-industri-terpadu', icon: Warehouse, color: '#8B0000' },
  { label: 'Kampung Pangan', href: '/nusa-futuristik/kota/kawasan-pangan-terpadu', icon: Wheat, color: '#16a34a' },
  { label: 'Kampung Sehat', href: '/nusa-futuristik/kota/wisata-terpadu', icon: Heart, color: '#dc2626' },
  { label: 'Kampung Cerdas', href: '/nusa-futuristik/kota/proyek-strategis', icon: GraduationCap, color: '#7c3aed' },
  { label: 'Kampung Hijau', href: '/nusa-futuristik/kota/proyek-strategis', icon: Flower2, color: '#059669' },
  { label: 'Kampung Wisata', href: '/nusa-futuristik/kota/rumah-produktif', icon: Home, color: '#92400e' },
  ]

const systems = [
  { title: 'MaaS (Mobility as a Service)', description: 'Platform terpadu yang menggabungkan semua moda transportasi kota dalam satu aplikasi.', icon: Zap, features: ['One App', 'Multi-Modal', 'Smart Fare'] },
  { title: 'Smart Public Transit', description: 'Sistem transportasi umum kota dengan real-time tracking dan predictive scheduling.', icon: Bus, features: ['Live Track', 'Smart Schedule', 'Crowd Data'] },
  { title: 'Bike & Scooter Sharing', description: 'Sistem berbagi sepeda dan skuter listrik untuk mobilitas last-mile yang ramah lingkungan.', icon: Bike, features: ['Dockless', 'E-Scooter', 'App Unlock'] },
  { title: 'Smart Parking System', description: 'Sistem parkir cerdas dengan sensor deteksi dan aplikasi pencarian slot parkir.', icon: Car, features: ['Sensor', 'App Find', 'Cashless'] },
  { title: 'Traffic AI Management', description: 'Pengelolaan lalu lintas berbasis AI untuk mengurangi kemacetan dan meningkatkan keselamatan.', icon: Navigation, features: ['AI Signal', 'Prediction', 'Accident Alert'] },
  { title: 'Connected Vehicle Network', description: 'Jaringan kendaraan terhubung melalui V2X untuk komunikasi real-time antar kendaraan.', icon: Wifi, features: ['V2X Comm', 'Safety Alert', 'Platooning'] },
]

export default function KotaTransportasiPage() {
  return (
    <main className="min-h-screen bg-white">
      <div className="bg-[#F5F5F5] border-b border-[#E5E7EB]">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm text-[#6B7280]">
            <Link href="/" className="hover:text-[#008F3D]">Beranda</Link>
            <span className="flex items-center gap-2"><ChevronRight className="w-4 h-4" /><Link href="/nusa-futuristik" className="hover:text-[#008F3D]">Nusa Futuristik</Link></span>
            <span className="flex items-center gap-2"><ChevronRight className="w-4 h-4" /><Link href="/nusa-futuristik/kota" className="hover:text-[#008F3D]">Kota</Link></span>
            <span className="flex items-center gap-2"><ChevronRight className="w-4 h-4" /><span className="text-[#1A1A1A] font-medium">Transportasi Digital</span></span>
          </nav>
        </div>
      </div>

      <section className="relative bg-gradient-to-r from-[#0d9488] to-[#0f766e] text-white py-12 md:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            <motion.div variants={fadeInUp}><Badge className="bg-white/20 text-white border-white/30 mb-4"><Zap className="w-3.5 h-3.5 mr-1.5" />KOTA FUTURISTIK</Badge></motion.div>
            <motion.h1 variants={fadeInUp} className="text-3xl md:text-4xl lg:text-5xl font-extrabold mb-4">Transportasi Digital Kota</motion.h1>
            <motion.p variants={fadeInUp} className="text-lg text-white/80 max-w-2xl mx-auto">Sistem mobilitas cerdas kota yang mengintegrasikan semua moda transportasi dalam platform digital terpadu.</motion.p>
          </motion.div>
        </div>
      </section>

      <section className="py-6 bg-[#F5F5F5] border-b border-gray-200">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-2">
            {siblingPages.map((page, i) => (
              <Link key={i} href={page.href}><Badge variant="outline" className="text-xs px-3 py-1.5 border-gray-300 text-gray-600 hover:border-[#0d9488] hover:text-[#0d9488] transition-colors cursor-pointer flex items-center gap-1.5"><page.icon className="w-3 h-3" />{page.label}</Badge></Link>
            ))}
          </div>
        </div>
      </section>

      <section className="py-12 md:py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="mb-8">
            <motion.h2 variants={fadeInUp} className="text-2xl md:text-3xl font-extrabold text-[#1A1A1A] mb-2">Sistem Transportasi Kota</motion.h2>
            <motion.p variants={fadeInUp} className="text-[#6B7280]">Berbagai solusi mobilitas cerdas untuk kota yang lebih efisien dan berkelanjutan.</motion.p>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {systems.map((sys, i) => (
              <motion.div key={i} variants={fadeInUp}>
                <Card className="border border-gray-100 hover:border-[#0d9488]/30 hover:shadow-lg transition-all duration-300 h-full">
                  <CardContent className="p-6">
                    <div className="w-12 h-12 rounded-xl bg-[#0d9488]/10 flex items-center justify-center mb-4"><sys.icon className="w-6 h-6 text-[#0d9488]" /></div>
                    <h3 className="font-bold text-[#1A1A1A] mb-2">{sys.title}</h3>
                    <p className="text-sm text-[#6B7280] leading-relaxed mb-4">{sys.description}</p>
                    <div className="flex flex-wrap gap-2">{sys.features.map((f, j) => (<Badge key={j} variant="secondary" className="text-xs bg-gray-100 text-[#6B7280]">{f}</Badge>))}</div>
                  </CardContent>
                </Card>
              </motion.div>
            ))}
          </motion.div>
        </div>
      </section>

      <section className="py-12 bg-gradient-to-r from-[#008F3D] to-[#006F30] text-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
            <motion.h2 variants={fadeInUp} className="text-2xl md:text-3xl font-extrabold mb-4">Menuju Smart Mobility Kota</motion.h2>
            <motion.p variants={fadeInUp} className="text-white/80 max-w-xl mx-auto mb-6">Bergabung dengan KMN BERDIKARI dan jadilah bagian dari revolusi transportasi digital kota Indonesia.</motion.p>
            <motion.div variants={fadeInUp}><Link href="/daftar"><Button className="bg-white text-[#008F3D] hover:bg-white/90 font-semibold shadow-lg group">Daftar Sekarang<ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" /></Button></Link></motion.div>
          </motion.div>
        </div>
      </section>
    </main>
  )
}
