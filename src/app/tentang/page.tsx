    ...
    <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden bg-[#8B0000]">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        >
          ...
          >
            Tentang{' '}
            <span className="text-[#00A847]">Koperasi Korporasi Multipihak Nusa Berdikari</span>
            <br />
          ...
          <Card className="bg-[#8B0000] border-[#008F3D]/30 border-2">
            <CardContent className="p-8">
              <div className="text-center mb-8">
                <Badge className="bg-[#008F3D] text-white mb-3">17 Dewan Pendiri</Badge>
                <h3 className="text-2xl font-bold text-white mb-2">
                  Anggota Ganjil — Quorum & Legal
                </h3>
                <p className="text-white/80 max-w-2xl mx-auto">
                  Tujuh belas pendiri (angka ganjil) memenuhi persyaratan quorum pengambilan keputusan dengan prinsip simple majority (9:8) maupun supermajority (12:5).
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {DEWAN_PENDIRI.map((founder, i) => (
                  <motion.div
                    key={founder.id}
                    ...
                    className={cn(
                      ...
                      founder.status === 'vacant' ? 'bg-white/5 opacity-60' : 'bg-white/10'
                    )}
                  >
                    <p className="font-semibold text-white text-sm">{founder.name || '(Posisi Kosong)'}</p>
    ...
    <section className="py-20 bg-background">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.p variants={fadeInUp} className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Pengurus, Pengawas, Dewan Pembina, dan Dewan Penasihat yang berkomitmen untuk kemajuan koperasi
          </motion.p>
        </motion.div>
        ...
        >
          <motion.h3 variants={fadeInUp} className="text-xl font-semibold mb-6 flex items-center gap-2">
            <Crown className="w-5 h-5 text-[#B7791F]" />
            Dewan Pembina
            <Badge variant="outline" className="ml-2 text-xs border-[#B7791F] text-[#B7791F]">6 Anggota Prominan</Badge>
          </motion.h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TEAM_STRUCTURE.dewanPembina.map((member) => (
              <motion.div key={member.id} variants={scaleIn}>
                <Card className="card-hover-lift h-full border-l-4 border-l-[#B7791F]">
                  <CardContent className="p-6 text-center">
                    <Avatar className="w-20 h-20 mx-auto mb-4 border-2 border-[#B7791F]">
                      <AvatarImage src={member.photo} alt={member.name} />
                      <AvatarFallback className="bg-[#B7791F] text-white text-lg">
                        {member.name.split(' ').slice(0, 2).map(n => n[0]).join('')}
                      </AvatarFallback>
                    </Avatar>
                    <h4 className="font-semibold mb-1">{member.name}</h4>
                    <p className="text-sm text-[#8B0000] mb-2 font-medium">{member.position}</p>
                    <p className="text-xs text-muted-foreground mb-2">{member.bio}</p>
                    <Badge variant="outline" className="text-xs border-[#B7791F] text-[#B7791F]">
                      Pembina
                    </Badge>
                  </CardContent>
                </Card>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <motion.p variants={fadeInUp} className="text-white/90 text-lg mb-8 max-w-2xl mx-auto font-medium">
            Bergabunglah dengan 125.000+ anggota KMN BERDIKARI dan jadilah bagian dari transformasi ekonomi digital desa Indonesia
          </motion.p>
