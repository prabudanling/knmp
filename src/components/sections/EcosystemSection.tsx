          ...
          <Card className="bg-gradient-to-br from-gray-50 to-white border border-gray-100 shadow-sm">
            <CardContent className="p-8">
              <h3 className="text-lg font-semibold text-gray-900 text-center mb-6">
                Pentagon Kedaulatan — 5 KPA dengan distribusi suara proporsional (30+20+20+15+15)
              </h3>
                ...
                {KPA_DATA.map((kpa) => (
                  <div key={kpa.id} className="flex items-center gap-2">
                    <div 
                      ...
                      style={{ backgroundColor: kpa.color }}
                    />
                    <span className="text-sm text-gray-600 font-medium">{kpa.name} — {kpa.votingPower}%</span>
                  </div>
                ))}
              </div>

              {/* Doktrin Anti-Oligarki Note */}
              <div className="mt-6 flex items-center justify-center gap-2 text-sm text-gray-500">
                <ShieldCheck className="w-4 h-4 text-[#8B0000]" />
                <span>
                  <strong className="text-[#8B0000]">Doktrin Anti-Oligarki:</strong> Tidak ada kelompok yang bisa mendominasi — keputusan diambil secara demokratis
                </span>
              </div>
            </CardContent>
          </Card>
